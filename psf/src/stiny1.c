...
void Process_command_line( int argc, char *argv[], char *filename, paramstruct *paraminfo )
{
	...
	if ( argc < 2 )
	{
		printf( "stiny1 parameter_file [jitter=x] [wmag=x] [scan=x] [pfile=file][cfile=file]\n" );
		exit(0);
	}
	...
	{ 
		...
		else if ( Find_string(argv[i], "CFILE") != NULL )
		{
			/*The Stiny configure file*/
			sscanf( &argv[i][6], "%s", paraminfo->parmConfiglfile);
		}
		else
		{
...
/*---------------------------------------------------------------------------------------------*/
int Read_position_file( paramstruct *paraminfo, char *filename,  PARMCONFIGFILE *parm, char *errorMessage )
{
	FILE	*file;
	int	i, num_read;
	...
	if ( file == NULL )
	{
		sprintf(errorMessage, "ERROR : Could not open position list file %s\n", filename );
		return 0;
	}
	...
	while ( !feof(file) )
	{
		if ( i == MAX_POSITIONS )
		{
			sprintf(errorMessage, "ERROR : Only %d positions allowed.\n", MAX_POSITIONS );
			fclose( file );
			return 0;
		}
		...
		{
			sprintf(errorMessage, "ERROR : Problems reading position list file\n" );
			fclose( file );
			return 0;
		}
...
/*------------------------------------------------------------------------*/
int Ask_psf_size( paramstruct *paraminfo ,  PARMCONFIGFILE *parm, char *errorMessage )
{
...
*/
		if( diam < 3*paraminfo->det_pixel_size_arcsec_x || diam > max_size )
		{
			sprintf(errorMessage,  
                "Maximum computable PSF diameter = %.1f arcseconds\n "
				"Minimum recommended PSF diameter = %.2f arcseconds\n"
				"Nominal pixel size = %.2f x %.2f arcseconds\n"
				"please check the value of psfSize\n"
				"input psfSize = %.2f \n"
				"Max psfSize = %.2f \n"
				"Min psfSize = %.2f \n"
			, max_size, min_size*60.0
			, paraminfo->det_pixel_size_arcsec_x, paraminfo->det_pixel_size_arcsec_y  
            , diam, max_size, 3*paraminfo->det_pixel_size_arcsec_x);
			return 0;
		}
	}
	else
...
*/
		...
		{
			sprintf(errorMessage,  
                "Maximum computable PSF diameter = %.3f arcminutes\n"
	...
	//scanf( "%s", choice );
	choice[0] = parm->subsampledPSF;

	if ( choice[0] == 'Y' || choice[0] == 'y' )
	{
		subfactor = parm->subsampleFactor;
		if ( subfactor < 2 || subfactor > 10 )
		{
...
/*------------------------------------------------------------------------*/
int Ask_spectrum( paramstruct *paraminfo,  PARMCONFIGFILE *parm, char *errorMessage  )
{
	int	choice, i;
	float	tot;
	int flag = 1;
...
*/
	switch ( parm->objectSpectrum)
	{
		case 1 : flag = Blackbody( paraminfo, parm, errorMessage);
			 break;
		case 2 : flag = Power_law_nu( paraminfo, parm, errorMessage );
			 break;
		case 3 : flag = Power_law_lambda( paraminfo, parm, errorMessage );
			 break;
		case 4 : printf( "\nNOTE : Spectrum file must contain wavelength (microns)\n" );
			 ...
			 flag = Spectrum_file( paraminfo, parm, errorMessage );
			 break;
	       default : sprintf(errorMessage,  "ERROR : Invalid selection\n" );
			 flag = 0;
	}
...
/*------------------------------------------------------------------------*/
int  Ask_general( paramstruct *paraminfo ,  PARMCONFIGFILE *parm, char *errorMessage )
{
	int flag = 1;
	if ( paraminfo->num_waves > 1 ){
		flag = Ask_spectrum( paraminfo, parm, errorMessage );
		if(flag == 0)
			return 0;
	}
	flag = Ask_psf_size( paraminfo, parm, errorMessage );
	if(flag == 0)
		return 0;
	...
	//paraminfo->rootname  = parm->rootname;
    strcpy(paraminfo->rootname, parm->rootname);
	return 1;
}
...
int main( int argc, char *argv[] )
{
	...
	memset(&paraminfo, 0, sizeof(paramstruct));
	...
	memset(&parm, 0, sizeof(PARMCONFIGFILE));
	memset(errorMessage, 0, MAX_STRING);
	int flag = 1;
	flag = getParmFromConfFile(&paraminfo, &parm, errorMessage);
	if(flag == 0)
	{
	...
	//showParm(&parm);
	flag = checkParm1(&parm, errorMessage);
	if(flag == 0)
	{
		printf("%s\n", errorMessage);
		exit(0);
	...
	paraminfo.camera = parm.cameraType;

	while ( paraminfo.camera < 1 || paraminfo.camera > IRS_PEAKUP_RED )
	{
		printf( "---------- IRAC ----------     ------------ MIPS ---------- \n" );
		printf( "1) Channel 1 (3.5 microns)\n" ); 
		printf( "2) Channel 2 (4.5 microns)\n" ); 
		printf( "3) Channel 3 (6.3 microns) \n" ); 
		printf( "4) Channel 4 (8.0 microns)\n" );
		printf( "Selection : " );
	...
	flag = Read_pupil_file( &paraminfo, &parm, errorMessage );
	
	flag = checkParm2( &paraminfo, &parm, errorMessage);
	if(flag == 0)
	{
		printf("%s\n", errorMessage);
		exit(0);
	...
	flag = Ask_general( &paraminfo , &parm, errorMessage );
	if(flag == 0)
	{
		printf("%s", errorMessage);
		exit(0);
