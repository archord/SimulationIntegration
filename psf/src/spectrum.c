...
*	temperature.
*----------------------------------------------------------------------------------*/
int Blackbody( paramstruct *paraminfo,  PARMCONFIGFILE *parm, char *errorMessage )
{
	int   i;
	...
	if ( t < 1.0 || t > 900000.0 )
	{	
		sprintf(errorMessage, "ERROR: Temperature must be 1 - 900000 K **\n"); 
		return 0;
	}
...
*--------------------------------------------------------------------------*/
int Power_law_nu( paramstruct *paraminfo, PARMCONFIGFILE *parm, char *errorMessage )
{
...
*	curve prior to calling this routine.
*--------------------------------------------------------------------------*/
int Power_law_lambda( paramstruct *paraminfo, PARMCONFIGFILE *parm, char *errorMessage )
{
...
*----------------------------------------------------------------------------*/
int Spectrum_file( paramstruct *paraminfo, PARMCONFIGFILE *parm, char *errorMessage )
{
	FILE 	*file;
	...
	while ( file == NULL )
	{
		...
		if ( file == NULL )
		{
			sprintf( errorMessage, "ERROR : Could not open %s, please check!\n", paraminfo->spectrum_file );
			return 0;
		}
	}
	...
	do {
		...
		{
			sprintf(errorMessage, "ERROR : Error reading %s, please check the content!\n", paraminfo->spectrum_file);
			fclose(file);
			return 0;
		}
	} while ( line[0] == '#' );
	...
	{
 		sprintf(errorMessage,  "ERROR : First non-comment line in spectrum file must be one of the following : FNU, FLAM, JY, PHOTLAM\n");
		fclose(file);
		return 0;
	}
